...
for (let index = 0; index < graduates.length; index++) {
  universities.push(graduates[index]);
}

console.log(universities.sort((a,b) => (a.university > b.university) ? 1 : ((b.university > a.university) ? -1 : 0)));
...
for (let index = 0; index < graduates.length; index++) {
  ...
  if(graduates[index].university.includes('Uni')) {uni.push(graduates[index])}
  // The solution below could be worse because it uses undefined to avoid handling the else case.
  // graduates[index].university.includes('Uni') ? uni.push(graduates[index]) : undefined;
}
...
*/
const populationTotal = zooAnimals.reduce((totalPopulation, currentAnimal) => {
  return totalPopulation += currentAnimal.population;
}, 0);
